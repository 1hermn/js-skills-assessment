...
describe('DepartmentsService', () => {
  ...
  it('get departments should return a array of departments', async () => {
    expect((await service.getDepartments()).departments[0]).toBeInstanceOf(
      Department,
    );
  });
  ...
  it('add department should add department', async () => {
    const department = await service.add(
  ...
  it('add employee by body should add employee', async () => {
    ...
    const employee = await service.addEmployee(
      new Employee('test', 'test', new Date(), 'test', 'test', department),
      departmentId,
    );
  ...
  it('should throw error', async () => {
    expect(async () => await service.delete(departmentId)).rejects.toThrow(
      HttpException,
    );
  });
  it('should throw error when delete employee from wrong department', async () => {
    expect(
      async () => await service.deleteEmployee(employeeId, departmentId + 1),
